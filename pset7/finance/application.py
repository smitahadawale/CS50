...
@login_required
def index():
    portfolio_symbols = db.execute("SELECT shares, symbol,total \
...
def buy():
    ...
    if request.method=="GET":
        ...
        if not balance or float(balance[0]["cash"]) < stock["price"] * shares:
...
def sell():
    ...
    if request.method=="GET":
        ...
        # select user's cash
        balance = db.execute("SELECT shares FROM portfolio WHERE id = :id and symbol=:symbol", \
                         id=session["user_id"],symbol=stock["symbol"])
        #share=shares-balance
        # check if enough money to buy
        if not balance or int(balance[0]["shares"]) <  shares:
        ...
        # if user doesn't has shares of that symbol, create new stock object
        shares_count=balance[0]["shares"]-shares
